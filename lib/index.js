...
//https://gist.github.com/Alex-Werner/d1641cb797a3cda0e6083c27522dc832
...
internals.implementation = function (server, options) {
    ...
    var scheme = {
        ...
        authenticate: function (request, reply) {
            ...
            var authorization = request.headers.cookie;

            if (!authorization) {
                return reply(Boom.unauthorized(null, 'Access Token'));
            }
    
            var parts = parseCookies(authorization);
            var access_token = parts['access_token'];
            if(!access_token) {
                return reply(Boom.unauthorized(null, 'Access Token'));
            }
            ...
            if (parts.length !== 2) {
                return reply(Boom.badRequest('Bad HTTP cookie header format', 'Access Token'));
            }
    
            if (parts[0].toLowerCase() !== 'access_token') {
                return reply(Boom.unauthorized(null, 'Access Token'));
            }
